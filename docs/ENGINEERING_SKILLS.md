...
| `to-tickets` | Adopt for work larger than one run | Use vertical tracer-bullet tickets and native blocking edges before the factory when a program cannot fit one approved run. Each ready ticket becomes one factory request. Do not add a second dependency scheduler inside the controller. |
